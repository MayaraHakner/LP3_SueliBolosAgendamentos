...
private List<Pedido> lista = new ArrayList<>();    public DAOPedido(){
    ...
    public List<Pedido> listById(int id) {
        return em.createQuery("SELECT e FROM Pedido + e WHERE e.nome= :id").setParameter("id", id).getResultList();
    }

    public List<Pedido> listInOrderNome() {
        return em.createQuery("SELECT e FROM Pedido e ORDER BY e.nome").getResultList();
    }
